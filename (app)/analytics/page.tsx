...
export default async function AnalyticsPage() {
  ...
  // --- DIAGNOSTIC: remove once chart is confirmed working ---
  console.log("[analytics] trendRaw type:", typeof trendRaw, Array.isArray(trendRaw) ? `array(${(trendRaw as unknown[]).length})` : String(trendRaw));
  if (Array.isArray(trendRaw) && (trendRaw as unknown[]).length > 0) {
    console.log("[analytics] trendRaw[0]:", JSON.stringify((trendRaw as unknown[])[0]));
  }
  // --- END DIAGNOSTIC ---
  ...
  console.log("[analytics] trend after map:", trend.length, "rows, first:", JSON.stringify(trend[0]));
